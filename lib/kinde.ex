defmodule Kinde do
  ...
  def token(state, code, config) when is_binary(state) do
    with {:ok, params} <- take_state(state) do
      token(params, code, params)
    end
  end

  def token({code_verifier: code_verifier, extra_params: extra_params}, code, config) do
    %{
      domain: domain,
      client_id: client_id,
    ...
    with {:ok, response} <- token_request(domain, params),
         {:ok, claims} <- get_token(response) do
      {:ok, user_params(claims), extra_params}
    end
  end

  defp token(%Tesla.Env{status: 200, body: body}) do
    body
    |> Map.fetch!("id_token")
    |> IdToken.verify_and_validate()
  end

  defp token(%Tesla.Env{status: status}) do
