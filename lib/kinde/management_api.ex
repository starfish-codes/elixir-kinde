defmodule Kinde.ManagementAPI do
  ...
  @impl GenServer
  def handle_call(:client, _from, %{access_token: access_token, business_domain: business_domain} = state) do
    client = Tesla.client([
  ...
  @impl GenServer
  def handle_info(:renew_token, %{client_id: client_id, client_secret: client_secret, business_domain: business_domain} = state) do
  ...
  defp init_state(client_id, client_secret, business_domain) do
    ...
    with {:ok, %Tesla.Env{status: status, body: body}} <- post(auth_client, "/oauth2/token", payload) do
  ...
  defp init_state(
    ...
    timeout = :timer.seconds(expires_in)

    state = %{
      client_id: client_id,
      client_secret: client_secret,
      access_token: access_token,
      renew_token_timer: schedule_renew_token(timeout)
    }
  ...
  defp schedule_renew_token(timeout),
    do: Process.send_after(self(), :renew_token, timeout)
end
